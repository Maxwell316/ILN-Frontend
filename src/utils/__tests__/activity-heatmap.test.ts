...
describe("activity-heatmap", () => {
  ...
  describe("formatActivityTooltip", () => {
    it("formats tooltip with count", () => {
      expect(
        formatActivityTooltip({ count: 3, totalAmount: 0n }, "2025-04-30"),
      ).toContain("3 actions");
    });

    it('uses singular "1 action" for a single event', () => {
      expect(
        formatActivityTooltip({ count: 1, totalAmount: 0n }, "2025-04-30"),
      ).toContain("1 action");
      expect(
        formatActivityTooltip({ count: 1, totalAmount: 0n }, "2025-04-30"),
      ).not.toContain("1 actions");
    });

    it('shows "No activity" for zero count', () => {
      expect(
        formatActivityTooltip({ count: 0, totalAmount: 0n }, "2025-04-30"),
      ).toContain("No activity");
    });

    it("includes total amount when non-zero", () => {
      const label = formatActivityTooltip(
        { count: 2, totalAmount: 1_000_000_000n },
        "2025-04-30",
      );
